...
// Use environment variable (set on Render)
...
// Helper: send email (only if API key is present)
async function sendEmail(to, subject, htmlContent) {
    ...
    try {
        const { data, error } = await resend.emails.send({
            from: 'Slice & Spice <onboarding@resend.dev>',
            to: [to],
            subject: subject,
            html: htmlContent
        });
...
app.post('/register', async (req, res) => {
    try {
        ...
        const user = { id: result.rows[0].id, email, full_name, role: 'customer' };
...
app.post('/orders', verifyToken, async (req, res) => {
    try {
        ...
        if (user && user.email) {
            const emailHtml = `
                <h2>Thank you for your order, ${user.full_name || 'Customer'}!</h2>
...
app.patch('/owner/orders/:id/status', verifyToken, async (req, res) => {
    ...
    try {
        ...
        const orderInfo = await query('SELECT user_id FROM orders WHERE id = $1', [id]);
        if (orderInfo.rows.length) {
            ...
            if (user && user.email) {
                let statusMessage = '';
                if (status === 'preparing') statusMessage = 'Your order is being prepared!';
                ...
                else statusMessage = `Your order status has been updated to ${status}.`;
                const emailHtml = `
                    <h2>Order #${id} status update</h2>
                    <p>Hello ${user.full_name || 'Customer'},</p>
                    <p>${statusMessage}</p>
                    <p>🍕 Slice & Spice</p>
                `;
                await sendEmail(user.email, `Order #${id} - ${status}`, emailHtml);
            }
        }
